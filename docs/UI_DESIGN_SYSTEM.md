...
Text size is a device-local presentation preference: Small (the original UI), Medium (1.16), and Large (1.32). The `Aa` utility immediately before Theme cycles these values and stores `put_scanner_text_size`. `UiTextSizeProvider` sets `data-text-size` on the document before paint; only the utility subscribes to its React context.
...
### Kinetic control and overlay motion

`--motion-instant` (90ms) handles presses and compact tooltips; `--motion-fast` (150ms) handles control feedback and backdrops; `--motion-medium` (210ms) handles disclosures. `--motion-reveal` (320ms) and `--ease-emphasized` (`cubic-bezier(0.16, 1, 0.3, 1)`) govern overlay entrances: modal 24px vertical with .985 scale, sheet 48px upward, drawer 64px lateral. Tooltips retain a separate 3px entrance. Individual CSS `translate`/`scale` preserve existing dialog centering transforms. Controls are usable during entrance.

Fine pointers lift enabled shared buttons and genuinely clickable instrument/market/heatmap cards by 2px. Pressed buttons reach .985 immediately, including short touch taps, and release over 90ms. The emphasized easing gives a quick response followed by a soft settle, without overshoot. Dense rows stay spatially still, with contact-edge feedback. Navigation and selected controls acknowledge selection with a growing two-pixel marker; icons and disclosure chevrons explain destination and expansion. Static KPI surfaces stay still; changed summary strings may fade in without interpolating financial values.

`useOverlayDismiss` coordinates user-requested dismissal of analysis, chart, evidence, Account, backup, maintenance and shared mobile sheets. CSS owns the 140ms exit; one bounded fallback timer handles missing animation events. The original dialog remains mounted with its focus trap and scroll lock until dismissal completes. Repeated closes are deduplicated; unmount or identity replacement cancels pending dismissal. Successful saves and workflow actions retain their existing immediate callbacks. Reduced motion bypasses dismissal waiting, including preference changes during an exit.

`motion-refresh-region` ties a non-looping accent edge to existing loading state. Its disappearance means activity ended, not that stale evidence became current. Chart data retains displayed-timeframe semantics; only the returned path receives a chronological clip reveal. Axes, warnings and labels remain immediate. `motion-disclosure` belongs on contained details, never whole tables or row lists.

All added spatial motion is inside `prefers-reduced-motion: no-preference`; the existing global reduced-motion rule remains intact. Glass stays on existing navigation/header surfaces with a solid fallback. No animation dependencies, acquisition effects, observers, per-row animation state, persistent loops or permanent `will-change` hints are added.
