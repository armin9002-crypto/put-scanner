...
- Headline metrics include Total Realized AY (Gross-Risk-weighted average of the existing simple annualized realized-yield formula), Avg. Days Held (one decimal), Wtd. Avg. Entry Delta (signed, with quiet Gross Risk coverage context), and Total Historical Notional (canonical Gross Risk across all History), alongside the existing resolved P&L/capture context.
