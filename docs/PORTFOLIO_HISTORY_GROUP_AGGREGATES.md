...
| Wtd. Avg. NY | Gross Risk | `sum(Entry NY × Gross Risk) / sum(known-NY Gross Risk)`. This follows the canonical History exposure convention. |
...
VIX @ Entry, Entry Delta, and Entry IV expose quiet coverage ratios:

```text
Gross Risk represented by known values / total valid group Gross Risk
```

Coverage is `null` when total valid Gross Risk is zero. Entry Delta and Entry IV expose their underlying known and total Gross Risk through their canonical helpers; headline and group consumers receive the same coverage ratios.
