...
|---|---|---|---:|---|
| Scanner | Initial/manual load -> one `/api/screener-expirations`, one `/api/prices`, one `/api/fund-metadata`, four `/api/price` charts | Session expiration cache plus `requestMarketData`/three-layer caches | 50: 42 option-metadata acquisitions, three 20-symbol price chunks, fund quote, and QQQ/SPY/VIX/VXN charts | Expiration membership is server-authoritative and partial failures remain unknown rather than hiding ETFs. Other data is cache-first; a newer price generation or route teardown aborts its consumers and suppresses stale publication. |
| Screener entry | Route entry -> `/api/screener-expirations` and VIX `/api/price` | Shared session expiration cache plus chart broker cache | Up to 43: 42 bounded option-metadata acquisitions + VIX | Entry calls receive route-teardown signals. Partial expiration discovery remains usable and is revalidated on the next entry. |
