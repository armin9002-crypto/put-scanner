...
| Nominal yield — discovery | `optionPrice / strike`; displayed as percent. | Scanner snapshots, Screener, detail rows, and Watchlist use `calculateYieldPercent`. | Denominator is gross secured cash per share. |
| Annualized yield — discovery | `(optionPrice / strike) × (365 / DTE)`; displayed as percent. | Scanner snapshots, Screener, detail rows, and Watchlist agree for the same quote and DTE. | Simple annualization, not compounded, and unavailable at DTE `<= 0`. It can exaggerate very short-duration economics. |
...
| IV Rank (current implementation) | Current ATM put IV is compared with a one-year series of four-week realized volatilities calculated from weekly closes. Rank is `(current ATM IV − min realized vol) / (max − min)`, clamped 0–100. “IV percentile” is the fraction of realized-vol observations below ATM IV. | `api/_lib/ivRank.js`; displayed on detail and Screener. | This is **not conventional IV Rank or IV Percentile**, because the historical series is realized volatility, not historical implied volatility. Rename to “IV vs 1Y realized-vol range” or acquire true historical IV before marketing it as IV Rank. |
