...
|---|---|---|---|
| DTE | Listed expiration calendar date minus current America/New_York market calendar date, in calendar days. Original portfolio DTE remains expiration date minus sold date. | Shared `calculateDte`; Scanner/Screener/detail/Watchlist use exact expiration dates and the canonical market date. Portfolio clamps remaining DTE to `max(0, DTE)` for its presentation methodology. | Expiration day is `0 DTE`, not expired; the next calendar day is negative. Weekends and holidays count. |
...
| Delta | Provider delta when usable; otherwise the versioned deterministic Black–Scholes put delta `N(d1) − 1`, with annualized time `DTE/365` and a 4.5% annual risk-free rate. | Detail, Scanner snapshot, Screener, Watchlist, and Portfolio refresh use the canonical resolver where applicable; current contract provenance remains transient. | Calculated Delta is tagged `black-scholes-put-delta-v1`; required inputs, including IV, must be valid and positive. There is no IV fallback. Delta is not a guaranteed probability of assignment. |
...
| Vol/OI | `volume / openInterest` when Volume is finite and non-negative and Open Interest is finite and greater than zero. Zero Volume is valid and yields `0`; otherwise the metric is unavailable. | Detail/Screener optional column. | Same-day volume and accumulated open interest answer different questions; high ratio is not automatically bullish or liquid. |
