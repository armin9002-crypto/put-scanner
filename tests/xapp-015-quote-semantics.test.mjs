...
test('XAPP-015 route parity: clean positive quote has one shared basis contract', () => {
  ...
  for (const result of states) {
    ...
    assert.deepEqual(result.defaultSoldPrice, { basis: 'last', value: 2.1 });
  }
  assert.deepEqual(states.map(result => ({ bid: result.trustedBid, ask: result.trustedAsk, mid: result.mid, default: result.defaultSoldPrice })), Array(5).fill({ bid: 2, ask: 2.2, mid: 2.1, default: { basis: 'last', value: 2.1 } }));
});
...
test('XAPP-015 trusted stale and unknown-age Last remain visibly stale but are the default basis', () => {
  for (const lastTradeDate of [STALE_LAST, '2026-08-20T15:00:00Z', null]) {
    ...
    assert.deepEqual(result.defaultSoldPrice, { basis: 'last', value: 2 });
  }
  ...
  assert.deepEqual(selectDefaultSoldPrice({ last: 2, lastTradeDate: RECENT_LAST, bid: null, ask: null }), { basis: 'last', value: 2 });
  assert.deepEqual(selectDefaultSoldPrice({ last: 2, lastTradeDate: STALE_LAST, bid: null, ask: null }), { basis: 'last', value: 2 });
  assert.deepEqual(state({ last: 2, lastTradeDate: STALE_LAST, bid: 1.8, ask: 2.2, integrityStatus: 'clean' }).defaultSoldPrice, { basis: 'last', value: 2 });
  assert.deepEqual(state({ last: 2, lastTradeDate: null, bid: 1.8, ask: 2.2, integrityStatus: 'clean' }).defaultSoldPrice, { basis: 'last', value: 2 });
});

test('XAPP-015 Last falls back to Bid, then Mid, and never auto-selects Ask', () => {
  assert.deepEqual(state({ last: null, lastTradeDate: null, bid: 1.8, ask: 2.2, integrityStatus: 'clean' }).defaultSoldPrice, { basis: 'bid', value: 1.8 });
  assert.deepEqual(state({ last: null, lastTradeDate: null, bid: null, ask: 2.2, integrityStatus: 'clean' }).defaultSoldPrice, null);
  assert.deepEqual(state({ last: null, lastTradeDate: null, bid: 1.8, ask: 2.2, integrityStatus: 'clean', lastFallbackOnly: true }).defaultSoldPrice, { basis: 'bid', value: 1.8 });
  assert.deepEqual(state({ last: null, lastTradeDate: null, bid: null, ask: null, integrityStatus: 'clean', lastFallbackOnly: true }).defaultSoldPrice, null);
});
...
test('XAPP-015 invalid quotes fail closed while degraded positive quotes remain usable with status', () => {
  ...
  assert.deepEqual(degraded.defaultSoldPrice, { basis: 'last', value: 2.1 });
});
