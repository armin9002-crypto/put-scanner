...
export default function RecommendationsPage() {
  ...
  return (
    <div className="recommendations-page motion-refresh-region min-h-screen overflow-x-hidden" data-refreshing={loading} style={{ backgroundColor: 'var(--bg)' }}>
        ...
        {!run ? (
          ...
          <>
            ...
            <section className="recommendations-board-section surface-card">
              ...
              {boardRows.length > 8 && <button type="button" className="recommendations-board-show-all pressable" onClick={() => setShowAllBoardRows(value => !value)}>{showAllBoardRows ? 'Show top 8' : `Show all ${boardRows.length}`}<ChevronDown className={`motion-chevron h-4 w-4 ${showAllBoardRows ? 'rotate-180' : ''}`} /></button>}
            </section>
          </>
        )}
      </div>
...
function FrontierRows({ run, ticker, onEvidence }: { run: RecommendationRun; ticker: string; onEvidence: (candidateId: string) => void }) {
  ...
  if (candidates.length === 0) return <div className="recommendations-frontier-empty motion-disclosure">Rejected before contract acquisition or no valid frontier contract.</div>;
  return <div className="recommendations-frontier-list motion-disclosure">{candidates.map(candidate => <button type="button" key={candidate.id} onClick={() => onEvidence(candidate.id)}><span className="font-mono font-semibold">${candidate.strike.toFixed(2)}P · {candidate.dte} DTE</span><span>{compactCandidateLine(candidate)} · {delta(candidate.economics.delta)} Δ · {percent(candidate.economics.moneynessPct, 0)} OTM</span><VerdictBadge verdict={candidate.verdict} /></button>)}</div>;
}
