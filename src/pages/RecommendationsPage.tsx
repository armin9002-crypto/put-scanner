...
function RecommendationCard({
  ...
  const headlineAy = candidate.economics.annualizedYieldBidPct != null
    ...
    <article className="recommendation-card surface-card">
      <button type="button" className="recommendation-card__summary" onClick={onEvidence}>
        <div className="min-w-0">
          ...
          <div className="recommendation-card__hero-metric"><strong>{headlineAy}</strong><span>{candidate.economics.annualizedYieldBidPct != null ? 'AY at displayed bid' : ayRange ? 'indicative AY range' : 'AY unavailable'}</span></div>
      ...
      <section className="recommendation-methodology-modal" role="dialog" aria-modal="true" aria-labelledby="recommendation-methodology-title">
        ...
        <div className="recommendation-methodology-modal__content">
          <div className="recommendation-methodology-grid">
            <article><strong>Regime + posture</strong><p>{run.market.regime.label} · {run.market.posture.label}. Posture DTE is context and compensation input, not a hard candidate veto.</p></article>
            <article><strong>Acquisition universe</strong><p>{run.universe.onlyEvaluateAtLeast60Dte ? 'Only expirations at least 60 DTE' : 'Shorter expirations allowed'}, bounded at {run.universe.maximumDte} DTE.</p></article>
            <article><strong>Expiration planner</strong><p>At most {run.universe.maxExpirationsPerUnderlying} unique representative expirations per qualified underlying: nearest, middle, and farthest eligible.</p></article>
            <article><strong>Underlying filter</strong><p>Only severe trend/regime hard-fails are removed before chains. Eligible and Watch setups retain explicit evidence quality.</p></article>
            <article><strong>Policy + pricing</strong><p>Absolute and relative yield hurdles use direct seller bids or clearly labeled same-expiration indicative ranges. Last never becomes seller credit.</p></article>
            <article><strong>Dominance + duration</strong><p>Comparable tenors use material AY, Delta, cushion, pricing, and actionability. Cross-tenor comparisons separately test whether extra duration earns compensation or defensive value.</p></article>
            <article><strong>Skeptic + robustness</strong><p>A structured skeptic states the strongest objection. Small explicit hurdle, risk, and price scenarios test whether the conclusion is stable.</p></article>
            <article><strong>Versions</strong><p>Engine v{run.engineVersion} · Policy v{run.policyVersion}. The snapshot, universe, provenance, trace, and outputs are exportable and replayable.</p></article>
          </div>
...
function HowRecommendationsWork({ run, onMethodology }: { run: RecommendationRun; onMethodology: () => void }) {
  const visibleStages = run.decisionTrace.stages.filter(stage => ['TRACKED_UNDERLYINGS', 'CHAINS_ACQUIRED', 'CONTRACTS_EVALUATED', 'POLICY_SURVIVORS', 'SURFACED_RECOMMENDATIONS'].includes(stage.key));
  return (
    <section className="recommendations-how surface-card">
      <div className="recommendations-how__heading"><div><span>How Recommendations Work</span><p>One bounded market snapshot moves through explicit evidence, policy, comparison, skeptic, and robustness gates.</p></div><button type="button" className="button-secondary" onClick={onMethodology}><Info className="h-3.5 w-3.5" />Full Methodology</button></div>
      ...
      <div className="page-frame page-frame--wide">
        ...
        {!run ? (
          <section className="recommendations-ready-state surface-card">
            <div className="recommendations-ready-state__mark">MARKET MODE · V1.1</div>
          ...
          <>
            ...
            {surfaced.length > 0 && <section className="recommendations-primary-section"><SectionHeader title="Top Opportunities" description="Only distinct contracts surfaced by the deterministic policy. Open the evidence view for the complete audit trail." /><div className="recommendations-primary-grid">{surfaced.map(({ selection, candidate }) => <RecommendationCard key={`${selection.class}-${candidate.id}`} candidate={candidate} className={selection.class} watched={watchIds.has(makeWatchlistId(candidate.ticker, candidate.expiration, candidate.strike))} onEvidence={() => setEvidenceCandidateId(candidate.id)} onOpen={() => openContract(candidate)} onWatch={() => toggleWatch(candidate)} />)}</div></section>}

            <HowRecommendationsWork run={run} onMethodology={() => setShowMethodology(true)} />
