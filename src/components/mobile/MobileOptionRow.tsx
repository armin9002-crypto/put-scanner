    ...
    <article className="pressable mobile-option-row">
      ...
      <div className="pointer-events-none relative z-10">
        ...
        <div className={`mt-1.5 grid gap-1 text-[10px] ${props.showNominalYield ? 'grid-cols-5' : 'grid-cols-4'}`}>
          <span className="truncate" title="Annualized Yield"><span style={{ color: 'var(--text-dim)' }}>AY </span><b className="font-mono" style={{ color: 'var(--accent-light)' }}>{percent(props.annualYield)}</b></span>
          {props.showNominalYield && <span className="truncate" title="Nominal Yield"><span style={{ color: 'var(--text-dim)' }}>NY </span><b className="font-mono" style={{ color: 'var(--text-secondary)' }}>{percent(props.nominalYield)}</b></span>}
