  ...
  return (
    <section className="mobile-expiration-group">
      <button type="button" onClick={onToggle} aria-expanded={expanded} className="pressable flex min-h-[64px] w-full items-start gap-2 px-3 py-2.5 text-left">
        <ChevronRight className={`motion-chevron mt-0.5 h-4 w-4 flex-none ${expanded ? 'rotate-90' : ''}`} />
      ...
      {expanded && <div className="motion-disclosure border-t" style={{ borderColor: 'var(--border)' }}>{children}</div>}
    </section>
  );
}
