...
export default function DataBackupModal({ onClose, onImported }: DataBackupModalProps) {
  ...
  const replaceCurrentData = () => {
    if (!pendingBackup || !recoveryDownloaded) return;
    try {
      applyPutScannerBackup(localStorage, pendingBackup);
      onImported();
      setPendingBackup(null);
      setRecoveryDownloaded(false);
      setError('');
      setSuccess('Backup imported. Portfolio, watchlist, and included preferences were replaced locally.');
    } catch (importError) {
      setSuccess('');
      setError(errorMessage(importError));
    }
      ...
      <section className="max-h-[94dvh] w-full overflow-y-auto rounded-t-2xl p-4 shadow-2xl sm:max-w-xl sm:rounded-2xl sm:p-5" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} role="dialog" aria-modal="true" aria-labelledby="data-backup-title">
        <header className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5" style={{ color: 'var(--text-muted)' }}>Export or restore durable local data. Market-data caches are never included.</p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <section className="rounded-xl p-3" style={{ backgroundColor: 'var(--surface-alt)', border: '1px solid var(--border)' }}>
            ...
            <button type="button" onClick={() => exportCurrentData()} className="pressable mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-3 text-xs font-semibold text-white" style={{ backgroundColor: 'var(--accent)' }}><Download className="h-4 w-4" /> Download Backup</button>
          </section>
          ...
          <section className="rounded-xl p-3" style={{ backgroundColor: 'var(--surface-alt)', border: '1px solid var(--border)' }}>
            ...
            <p className="mt-2 text-xs leading-5" style={{ color: 'var(--text-muted)' }}>A file is parsed and validated before any local data can change.</p>
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="pressable mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-3 text-xs font-semibold" style={{ color: 'var(--text)', backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }}><Upload className="h-4 w-4" /> Choose Backup File</button>
          </section>
        </div>
        ...
        {summary && pendingBackup && (
          <section className="mt-4 rounded-xl p-4" style={{ backgroundColor: 'var(--bg)', border: '1px solid var(--accent-border)' }}>
            ...
            <div className="mt-4 rounded-lg p-3" style={{ backgroundColor: 'rgba(250,204,21,0.08)', border: '1px solid rgba(250,204,21,0.24)' }}>
              <div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 flex-none" style={{ color: 'var(--yellow)' }} /><p className="text-xs leading-5" style={{ color: 'var(--text-muted)' }}><b style={{ color: 'var(--text)' }}>Replace Current Data</b> replaces the durable data in this browser. Download the required recovery copy first.</p></div>
            ...
            <button type="button" disabled={!recoveryDownloaded} onClick={replaceCurrentData} className="pressable mt-3 min-h-11 w-full rounded-lg px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40" style={{ backgroundColor: 'var(--red)' }}>Replace Current Data</button>
          </section>
        )}
      </section>
    </div>
