...
const guardrails = [
  ...
  ['Recommendations board, sorting, expansion, near misses, evidence, and export are in-memory interactions', sources.recommendations.includes('buildBoardRows(run, boardSort)') && sources.recommendations.includes('setExpanded') && sources.recommendations.includes('run.nearMisses') && sources.recommendations.includes('JSON.stringify(run') && !/fetch\s*\(/.test(sources.recommendations) && !/fetch\s*\(/.test(sources.recommendationEvidence)],
