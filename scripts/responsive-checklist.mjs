...
const guardrails = [
  ...
  ['Portfolio History exposes six rolling and two point-in-time analytics metrics with dynamic width and touch inspection', sources.portfolio.includes('<RollingHistoricalAnalyticsChart trades={rollingTrades} />') && sources.rollingChart.includes('buildRollingHistoricalAnalyticsSeries') && sources.rollingChart.includes('buildPortfolioHistoricalStateSeries') && sources.rollingChart.includes('ROLLING_HISTORICAL_METRIC_CONFIGS.map') && sources.rollingChart.includes('PORTFOLIO_HISTORICAL_STATE_METRIC_CONFIGS.map') && sources.rollingChart.includes('ROLLING_WINDOW_MONTHS.map') && sources.rollingChart.includes('onPointerDown') && sources.rollingChart.includes('ResizeObserver') && sources.rollingChart.includes('preserveAspectRatio="none"') && sources.rollingChart.includes('data-rolling-domain-start') && !/fetch\s*\(|localStorage|sessionStorage|supabase/i.test(sources.rollingChart) && sources.css.includes('.rolling-historical-analytics')],
