...
const guardrails = [
  ...
  ['Portfolio History exposes one local rolling analytics chart with seven configured metrics and touch inspection', sources.portfolio.includes('<RollingHistoricalAnalyticsChart trades={rollingTrades} />') && sources.rollingChart.includes('buildRollingHistoricalAnalyticsSeries') && sources.rollingChart.includes('ROLLING_HISTORICAL_METRIC_CONFIGS.map') && sources.rollingChart.includes('ROLLING_WINDOW_MONTHS.map') && sources.rollingChart.includes('onPointerDown') && sources.rollingChart.includes('data-rolling-domain-start') && !/fetch\s*\(|localStorage|sessionStorage|supabase/i.test(sources.rollingChart) && sources.css.includes('.rolling-historical-analytics')],
