...
const guardrails = [
  ...
  ['Options hide Nominal Yield by default and expose it without changing row density', sources.options.includes('useState(readShowNominalYield)') && sources.options.includes('showNominalYield && <td') && sources.mobileOptionRow.includes('showNominalYield') && sources.options.includes('py-1.5')],
